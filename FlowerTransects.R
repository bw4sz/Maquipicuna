...
tax<-gnr_resolve(names = Families,canonical = T,best_match_only = T)
...
full.fl$Genus<-paste(toupper(substring(word(full.fl$Genus),1,1)),substring(word(full.fl$Genus),2),sep="")
full.fl$Species<-tolower(full.fl$Species)
...
tax<-gnr_resolve(names = Species[-1],canonical = T,best_match_only = T)
...
print(ggplot(fl.totals[,],aes(x=Month.a,log(TotalFlowers),shape=Year)) + geom_point(size=3) + stat_smooth(aes(group=1),method="glm", method.args=list(family="quasipoisson"), formula = y ~ ns(x, 3)) + theme_bw()  + ylab("Available Flowers") + xlab("Month") + facet_wrap(~Elev,scales="free_y") )
